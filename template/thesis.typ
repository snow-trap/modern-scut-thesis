...
本章说明如何用本 Typst 模板实现 SCUT 规范中的各项格式。运行环境（安装 Typst、准备字体、创建项目）见仓库 README 的「使用」一节；Typst 的定位与入门资料见 README 的「优势」一节，以及小蓝书@raindrop-blue 与 Typst 中文社区导航@typst-guide-cn。
