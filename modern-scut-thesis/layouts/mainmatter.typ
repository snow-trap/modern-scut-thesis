...
#let mainmatter(
  ...
  if leading == auto {
    // 1.5倍行距: baseline距离 = 1.5 × 字号 (12pt) = 18pt
    // typst leading = 行间距 = 18pt - 12pt = 6pt = 0.5em
    set par(
      leading: 0.5em,
      justify: justify,
  ...
  // 页眉五号宋体居中，下加 1.5 磅实线 (参考 scutthesis.tex 的 fancyhdr 设置)
      ...
      header: context {
        ...
        if not skip-on-first-level or cur-heading == none {
          if header-render == auto {
            ...
            // SCUT: 偶数页学校名，奇数页章标题
            // 对应 LaTeX: \fancyhead[CE]{学校名} \fancyhead[CO]{\leftmark}
            ...
            align(center, stack(
              header-text,
              v(0.25em),
              // SCUT: \renewcommand{\headrulewidth}{1.5pt}
