...
公式居中书写，序号按章编排（如第一章第一个公式为 "(1-1)"）。
独立编号的公式直接写，引用用 `@eqt:linear`，如@eqt:linear：

$ y = a x + b $ <linear>

若块级公式是前段文字的自然延续（不另起段落），
在段末加 `\ ` 换行，公式用 `#box(width: 100%)` 包裹，
避免后续文字出现首行缩进：
